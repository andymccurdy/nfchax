...
def get_usb_port_path(device):
    """Return the USB bus-port path (e.g. '1-1') for a /dev/ttyUSBX device."""
    try:
        ...
        for line in out.splitlines():
            m = re.search(r'KERNELS=="(\d+-[\d.]+)"', line)
...
def build_udev_rules(port_map):
    """Return udev rules content pinning /dev/nfcN to each physical USB port."""
    lines = ["# Generated by nfchax/one_time_setup.py — pins /dev/nfcN to physical USB ports."]
    for i, (_device, port_path) in enumerate(port_map):
        lines.append(
            f'SUBSYSTEM=="tty", SUBSYSTEMS=="usb", '
            f'ATTRS{{idVendor}}=="{CH340_VENDOR}", ATTRS{{idProduct}}=="{CH340_PRODUCT}", '
            f'KERNELS=="{port_path}", SYMLINK+="nfc{i}"'
        )
    return "\n".join(lines) + "\n"


def install_udev_rules():
    """Discover USB-serial port paths, write udev rules creating /dev/nfcN symlinks."""
    usb_devs = sorted(glob.glob("/dev/ttyUSB*"))
    if not usb_devs:
        print("No /dev/ttyUSB* found — skipping udev rules installation.")
        return

    port_map = [(d, get_usb_port_path(d)) for d in usb_devs]
    port_map = [(d, p) for d, p in port_map if p]
    if not port_map:
        print("Could not read USB port paths — skipping udev rules installation.")
        return

    rules = build_udev_rules(port_map)
    existing = ""
    ...
    if existing:
        print(f"udev rules already installed: {UDEV_RULES_PATH} — skipping (delete to regenerate)")
        return
    ...
    if result.returncode != 0:
        print(f"Warning: could not write udev rules ({result.stderr.strip()})")
        print(f"  Run manually: sudo tee {UDEV_RULES_PATH} << 'EOF'")
        print(rules + "EOF")
        return
    ...
    subprocess.run(["udevadm", "settle"], capture_output=True)  # wait for symlinks
    print(f"Installed udev rules: {UDEV_RULES_PATH}")
    for i, (device, path) in enumerate(port_map):
        print(f"  /dev/nfc{i} → USB port {path} (currently {device})")
...
def candidate_devices():
    ...
    modules have none. Prefers /dev/nfcN symlinks (stable, port-pinned) over
    /dev/ttyUSBX (order-dependent) when udev rules have been installed."""
    candidates = []
    if os.path.exists("/dev/ttyAMA0"):
        candidates.append(("/dev/ttyAMA0", 20))
    nfc_devs = sorted(glob.glob("/dev/nfc[0-9]*"))
    if nfc_devs:
        for device in nfc_devs:
...
def _wifi_interface():
    """Return the first WiFi interface name reported by `iw dev`, or None."""
    try:
...
def disable_wifi_power_management():
    """Write a NetworkManager drop-in that disables WiFi power saving (persistent).

    Returns True if anything changed."""
    existing = ""
    if os.path.exists(WIFI_PM_CONF):
        try:
            with open(WIFI_PM_CONF) as f:
                existing = f.read()
        except OSError:
            pass

    if existing == WIFI_PM_CONTENT:
        print(f"{WIFI_PM_CONF} already disables WiFi power management — no change.")
        return False

    result = subprocess.run(
        ["sudo", "tee", WIFI_PM_CONF],
        input=WIFI_PM_CONTENT, capture_output=True, text=True,
    )
    if result.returncode != 0:
        print(f"Warning: could not write WiFi power management config ({result.stderr.strip()})")
        print(f"  Run manually: printf '[connection]\\nwifi.powersave = 2\\n' | sudo tee {WIFI_PM_CONF}")
        return False

    # Reload NM config without dropping connections, then apply immediately.
    subprocess.run(["sudo", "systemctl", "reload", "NetworkManager"], capture_output=True)
    iface = _wifi_interface()
    if iface:
        subprocess.run(["sudo", "iwconfig", iface, "power", "off"], capture_output=True)
        print(f"Disabled WiFi power management: {WIFI_PM_CONF} (applied to {iface})")
    else:
        print(f"Disabled WiFi power management: {WIFI_PM_CONF} (takes effect on reconnect)")
    return True
...
def main():
    ...
    print()
    print("Installing udev rules for stable /dev/nfcN device names...")
    install_udev_rules()
